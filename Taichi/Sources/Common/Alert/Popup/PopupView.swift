...
public struct Popup<PopupContent: View>: ViewModifier {
    init(
        ...
        disappearTo = params.disappearTo
        ...
        dragToDismissDistance = params.dragToDismissDistance
    ...
    public struct PopupParameters {
        ...
        public func centerPopup() -> PopupParameters {
            ...
            params.animation = .easeIn(duration: 0.2)
        ...
        public func bottomPopup() -> PopupParameters {
            var params = self
            params.position = .bottom
            params.animation = .easeIn(duration: 0.2)
            params.appearFrom = .bottomSlide
            params.backgroundColor = .black.opacity(0.3)
            params.closeOnTap = false
            params.closeOnTapOutside = false
            params.disappearTo = .bottomSlide
            params.isOpaque = true
            params.dragToDismiss = false
            params.useKeyboardSafeArea = true
            return params
        }
        ...
        public func backgroundView(_ backgroundView: () -> (some View)) -> PopupParameters {
            var params = self
    ...
    func hiddenOffset(_ appearAnimation: AppearAnimation) -> CGPoint {
        ...
        case .topSlide:
            CGPoint(x: displayedOffsetX, y: -presenterContentRect.minY - safeAreaInsets.top - sheetContentRect.height)
        case .bottomSlide:
            CGPoint(x: displayedOffsetX, y: screenHeight)
        case .leftSlide:
            CGPoint(x: -screenWidth, y: displayedOffsetY)
        case .rightSlide:
            CGPoint(x: screenWidth, y: displayedOffsetY)
        case .centerScale:
            CGPoint(x: displayedOffsetX, y: displayedOffsetY)
        }
    }
    ...
    private var calculatedAppearFrom: AppearAnimation {
        let from: AppearAnimation = if let appearFrom {
            appearFrom
    ...
    private var calculatedDisappearTo: AppearAnimation {
        let to: AppearAnimation = if let disappearTo {
            disappearTo
            ...
            if #available(iOS 17.0, tvOS 17.0, macOS 14.0, watchOS 10.0, *) {
                ZStack {
                    VStack {
                        contentView()
                            ...
                            .scaleEffect(actualScale) // scale is here to avoid it messing with frameGetter for
                        // sheetContentRect
                    }
                    ...
                    .onChange(of: shouldShowContent) { newValue in
                        if actualCurrentOffset == CGPoint
                            .pointFarAwayFromScreen { // don't animate initial positioning outside the screen
                            DispatchQueue.main.async {
                ...
                ZStack {
                    ...
                    .onChange(of: targetCurrentOffset) { newValue in
                        if !shouldShowContent,
                           newValue == hiddenOffset { // don't animate initial positioning outside the screen
                    ...
                    .onChange(of: targetScale) { newValue in
                        if !shouldShowContent,
                           newValue == hiddenScale { // don't animate initial positioning outside the screen
                            actualCurrentOffset = targetCurrentOffset
    ...
    func changeParamsWithAnimation(_ isDisplayAnimation: Bool) {
        actualCurrentOffset = isDisplayAnimation ? CGPoint(x: displayedOffsetX, y: displayedOffsetY) : hiddenOffset
