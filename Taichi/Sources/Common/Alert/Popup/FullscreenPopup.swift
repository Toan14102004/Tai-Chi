...
public struct FullscreenPopup<Item: Equatable, PopupContent: View>: ViewModifier {
    ...
    /// A temporary variable to hold a copy of the `itemView` when the item is nil (to complete `itemView`'s dismiss
    /// animation)
    ...
    init(
        ...
        isPresentedRef = ClassReference(self.$isPresented)
        itemRef = ClassReference(self.$item)
    }
